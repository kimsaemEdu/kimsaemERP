...
public class LoginSecurityService implements UserDetailsService {
	...
	// 조회한 사용자 정보를 UserDaetails를 변환해서 리턴하는 메소드
	@Override
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		// -> 변환해서 리턴하지 않고 UserDetails를 상속하는 DTO를 직접 만들어서 작업해도 좋다.
		// -> UserDetails를 구현하고 있는 User객체를 만들어서 리턴한다.
		// -> 1. User클래스는 id,password,권한 리스틀 ㄹ매개변수로 받고 있으며
		// -> 2. 권한 리스트는 여러 개가 될쑤 있으므로 ArrayLsit에 담아서 리턴
		// -> 3. ArrayList에 담는 권한 정보는 GrantedAuthority객체타입이어야
		// 하고 GrantedAuthority는 인터페이스 이므로 이 인터페이스를 구현하는
		// SimpleGrantedAuthority객체를 생성해서 ArrayList에 추가한다.
		...
		ArrayList<AuthorityDTO> list = (ArrayList<AuthorityDTO>) dao.findByGroup(loginUser.getId());
		for (int i = 0; i < list.size(); i++) {
			AuthorityDTO dto = list.get(i);
			authorities.add(new SimpleGrantedAuthority(dto.getAuthorityname()));
			System.out.println(dto.getAuthorityname());
		}
