...
public class LoginSecurityService implements UserDetailsService {
	...
	//dao의 메소드를 이용해서 입력받은 아이디에 해당하는 레코드가 있는지 확인한 후 조회한 사용자 정보를 UserDetails를 변환해서 리턴하는 메소드
	...
	public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
		...
		//1. User클래스는 id, password, 권한리스트를 매개변수로 받고 있으며 권한리스트는 여러개가 될 수 있으므로 ArrayList에 담아서 리턴
		//2. ArrayList에 담는 권한 정보는 GrantedAuthority 객체타입이어야 하고 GrantedAuthority는 인터페이스 이므로 이 인터페이스를
